...
namespace OGA.MSSQL
{
    ...
    /// </summary>
    public class SQL_Tools : IDisposable
    {
        #region Private Fields

        static private string _classname = nameof(SQL_Tools);

        static private volatile int _instancecounter;

        private OGA.MSSQL.SQL_DAL _dal;
            ...
            try
            {
                OGA.SharedKernel.Logging_Base.Logger_Ref?.Info("{0}: " +
                    "Attempting to get column names for table {2} of database {1}..."
                        , nameof(SQL_Tools), Database, tablename);
            ...
            try
            {
                ...
                OGA.SharedKernel.Logging_Base.Logger_Ref?.Info("{0}: " +
                    "Attempting to restore database {1} from file {2}..."
                        , nameof(SQL_Tools), database, filepath);
